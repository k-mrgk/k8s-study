...
type WorkflowLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta.Workflow, err error)
...
// List lists all Workflows in the indexer.
func (s *workflowLister) List(selector labels.Selector) (ret []*v1beta.Workflow, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta.Workflow))
	})
	return ret, err
}
...
type WorkflowNamespaceLister interface {
	...
	Get(name string) (*v1beta.Workflow, error)
...
// List lists all Workflows in the indexer for a given namespace.
func (s workflowNamespaceLister) List(selector labels.Selector) (ret []*v1beta.Workflow, err error) {
...
// Get retrieves the Workflow from the indexer for a given namespace and name.
func (s workflowNamespaceLister) Get(name string) (*v1beta.Workflow, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta.Resource("workflow"), name)
	}
	return obj.(*v1beta.Workflow), nil
}
